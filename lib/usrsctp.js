...
Socket.prototype.listen = function(options, callback) {
	...
	if (options.backlog !== 0) {
		...
		if (--usrsctp._activeSockets === 0) usrsctp._syscall.unref();
		this._listen = false;
		this.emit('unlisten');
	}
	return this;
...
Socket.prototype.close = function() {
	if (this._listen && this.--usrsctp._activeSockets === 0) usrsctp._syscall.unref();
	syscall.close(this._nativeSocket);
};

Socket.prototype.write = function(data, encoding, options) {
	var data = null,
		encoding = 'utf8',
		options = {},
		argc = 0;
	if (argument[argc] instanceof Buffer || typeof argument[argc] === 'string') data = arguments[argc++];
	if (typeof argument[argc] === 'string') encoding = arguments[argc++];
	if (typeof argument[argc] === 'object') options = arguments[argc++];
	
	if (data === null) throw Error('Invalid data');
	if (typeof data === 'string') data = new Buffer(data, encoding);
	options = _.chain(options)
		.pick('stream_id', 'unordered', 'ppid', 'assoc_id')
		.defaults({
			'stream_id': 0,
			'unordered': false,
			'ppid': 0,
	...
	if (this._style === 'stream' && options.assoc_id !== null) throw Error('No assoc id needed for STREAM socket');
	if (this._style === 'seqpacket' && options.assoc_id === null) throw Error('Assoc id needed for SEQPACKET socket');
	
	syscall.send(this._nativeSocket, options, data, 0, data.length);
};

Socket.prototype.end = function(assoc_id) {
	syscall.end(this._nativeSocket, assoc_id);
};

Socket.prototype.address = function(assoc_id) {
	return _.map(syscall.get_local_addr(this._nativeSocket, assoc_id),
		function(addr) {
			addr.family = net.isIP(addr.address);
		});
};

Socket.prototype.remoteAddress = function(assoc_id) {
	return _.map(syscall.get_remote_addr(this._nativeSocket, assoc_id),
		function(addr) {
			addr.family = net.isIP(addr.address);
		});
};
...
util.inherits(Association, EventEmitter);

Association.prototype._clear = function() {
	delete this.socket.associations[this.assoc_id];
};

Association.prototype.write = function(data, encoding, options) {
	var data = null,
		encoding = 'utf8',
		options = {},
		argc = 0;
	...
	if (typeof argument[argc] === 'string') encoding = arguments[argc++];
	if (typeof argument[argc] === 'object') options = arguments[argc++];
	options.assoc_id = this.assoc_id;
	this.socket.write(data, encoding, options);
};

Association.prototype.end = function() {
	this.socket.end(this.assoc_id);
};

Association.prototype.address = function() {
	return this.socket.address(this.assoc_id);
};

Association.prototype.remoteAddress = function() {
	return this.socket.remoteAddress(this.assoc_id);
};
